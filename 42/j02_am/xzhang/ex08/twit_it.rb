...
when "--post"
	...
	if msg.size>120
		puts "Votre message est trop long (>120 caracteres)"
	else
		...
		query = URI.encode_www_form('status' => ARGV[1])
		address += "?#{query}"
...
when "--search"
	...
	query = URI.encode_www_form("q" => ARGV[1],"count" => 10)
	address += "?#{query}"
	puts address
	request = Net::HTTP::Get.new address.request_uri
	request.oauth! http, consumer_key, access_token
